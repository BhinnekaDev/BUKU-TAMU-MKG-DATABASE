...
function getClientIp(req) {
  return req.headers['x-forwarded-for'] || req.socket?.remoteAddress?.replace('::ffff:', '') || 'UNKNOWN';
}
...
app.post('/register/pengunjung', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Register pengunjung sukses', user: result.rows[0] });
  } catch (err) {
...
app.post('/logout/pengunjung', (req, res) => {
  // Jika menggunakan token di client, cukup dihapus dari penyimpanan lokal
